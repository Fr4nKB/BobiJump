<?php  
	...
	$BobiJumpDB = new BobiJumpDBManager();
	
	class BobiJumpDBManager {
		private $mysqli_conn = null;
	
		function BobiJumpDBManager(){
			$this->openConnection();
		}
		...
		//funzione che permette di verificare se una query di tipo update ha dato esito positivo
		function affectedRows($queryText){
			if (!$this->isOpened())
				$this->openConnection();
			
			$this->mysqli_conn->query($queryText);
